...
if (empty($_POST['value']) || ctype_space($_POST['value']))
   ...
   $query = "SELECT * FROM posts INNER JOIN users ON posts.id_Users = users.id_Users WHERE posts.tags_Posts LIKE '%$_POST[value]%' OR  posts.title_Posts LIKE '%$_POST[value]%' ;";
...
if (!$result) {
   ...
   }else echo '<p class="paragraph mx-auto mt-5">No posts found</p>';
}
